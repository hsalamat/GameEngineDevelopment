//Week4-8-Material2Demo
//We simply changed our quad to have texture coordinates 
//that range from zero to two.This means that Ogre 3D needs 
...
OgreTutorial::OgreTutorial()
    : ApplicationContext("week4-8-Material3Demo")
{
}
...
void OgreTutorial::createScene()
{
    ...
    ManualObject->begin("MyMaterial3", RenderOperation::OT_TRIANGLE_LIST);
    ...
    ManualObject->position(5.0, 0.0, 0.0);
    ManualObject->textureCoord(0, 2);
    ManualObject->position(-5.0, 10.0, 0.0);
    ManualObject->textureCoord(2, 0);
    ManualObject->position(-5.0, 0.0, 0.0);
    ManualObject->textureCoord(2, 2);
    ManualObject->position(5.0, 10.0, 0.0);
    ManualObject->textureCoord(0, 0);
