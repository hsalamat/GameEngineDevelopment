/** @file RingEmitterDemo
 *  @brief we have created MySmoke14 particle system (MyParticles.particle under media/particle folder)
    Only emitting from one point is boring, using a box is much more fun.
    1. Define the Ring using width and height:
    height 50
    width 50
    2. Now, to create a ring and not a circle, we need to define how much of the inner
    part shouldn't emit particles. Here we use percentages:
    inner_height 0.9
    inner_width 0.9
    3. The rest stays untouched, as follows:
    emission_rate 50
    direction 0 1 0
    velocity 20
...
void Game::createScene()
{
    ...
    Ogre::ParticleSystem* partSystem = mScnMgr->createParticleSystem("Smoke", "MySmoke14");
