/** @file Week7-9-complexCompositorDemo2
 *  @brief Define the number of pixels in the material and not in the fragment shader
 *  @attention: Run with OpenGL 3+ 
 *  Create a new fragment shader (91) that has all the old parameters, a new one, which is
 *  uniform, and a float value named numpixels.
 *  @note You should see some pixels of different colors instead of the normal instance of Sinbad.
  fragment_program MyFragmentShader91 glsl  
    {
        source MyShadersFS91.glsl   
        syntax glsl150  
        profiles glsl
        default_params                    
        {        
...
Game::Game()
    : ApplicationContext("Week7-9-complexCompositorDemo2")
{
}
