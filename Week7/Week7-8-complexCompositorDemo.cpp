/** @file Week7-8-complexCompositorDemo
 *  @brief Have a compositor that can make our scene look like it has a lot less pixels than it actually does
 *  @attention: Run with OpenGL 3+
 *  In the fragment shader, First, we need two variables, namely, num and stepsize. 
 *  The variable stepsize is one divided by num:
 *  float num= 50;
 *  float stepsize = 1.0/ num;
 *  Then use both these variables and the texture coordinates to calculate the new
 *  texture coordinates:
 *  vec2 fragment = vec2(stepsize * floor(oUv0.x * num),stepsize *  floor(oUv0.y * num));
 *  Use the new texture coordinates for retrieving the color from the texture:
 *  fragColour = texture(RT,fragment);  
 *  @note You should see some pixels of different colors instead of the normal instance of Sinbad.
...
Game::Game()
    : ApplicationContext("Week7-8-complexCompositorDemo")
{
}
