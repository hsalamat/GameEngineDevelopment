/** @file Week7-6-CombiningCompositorsInCodeDemo
 *  @brief Combining Compositors in code
 *  @attention: Run with OpenGL 3+
 *
 * In the last two examples, we have seen how we can create more complex compositors by
 * combining several render steps with different materials. We had to write a new compositor
 * script to combine several render steps, even though we already had two compositors,
 * each one describing one half of the final effect. Wouldn't it be nice to combine these two
 * compositors to create the final effect without the need to write a new compositor script
 * each time we want to combine some compositors? This is exactly what we are going to
 * accomplish in the next example: making use of the compositor chain.
 * 
...
Game::Game()
    : ApplicationContext("Week7-6-CombiningCompositorsInCodeDemo")
{
}
...
void Game::createScene()
{
    ...
    //Let's combine compositors in code 
