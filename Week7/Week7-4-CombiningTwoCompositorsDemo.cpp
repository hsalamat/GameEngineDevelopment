/** @file Week7-4-CombiningTwoCompositorsDemo
 *  @brief To combine two compositors, we need to create a new one
 *  @attention: Run with OpenGL 3+
 *
 * We created a second auxiliary texture; this texture was used as the render target for the
 * black and white material, and then this texture with the black and white image of our scene
 * was used as an input for the invert material, which then got rendered to the display.
 *
 *
    1. To create a new compositor, we need two textures—one for storing
    the scene and one for storing some temporary results:
    ...
    2. Fill the scene texture, as done previously, and then fill the temp texture
    using the scene texture and our black and white material:
    target scene
    {
    input previous
    }
    ...
    3. Then use the temporary material and our invert material to create the
    output texture:
    target_output
    {
    input none
...
Game::Game()
    : ApplicationContext("Week7-4-CombiningTwoCompositorsDemo")
{
}
